...
so a specificity, a PPV or a patient-level ROC-AUC were not measurable at all (DOCUMENTATION.md,
...
GroupNorm stack, not BatchNorm (see DOCUMENTATION.md §4.1 on why BatchNorm collapses here), and
...
----------------------------
89 cancer patients exist in the whole BCS-DBT collection (DOCUMENTATION.md); this corpus holds
...
in an optimistic number (DOCUMENTATION.md, "Écarts doc <-> code").

Patient level, not exam level
-------------------------------
A patient can contribute up to four views (exams). The score reported is per
**patient** -- the level DOCUMENTATION.md's target is stated at -- taking the max over that
...
reproducible (DOCUMENTATION.md, "Prochaines pistes pour l'étape 1")
------------------------------------------------------------------------------------
...
def bag_logit(logits, k=1, relative=False):
    ...
    the fix for a measured design error (DOCUMENTATION.md §4.9). The quantity that separates a
...
def warm_start_encoder(model, bank, train_exams, args, device, fold):
    ...
    Why: DOCUMENTATION.md §4.7 measured that the MIL loss does not fall in *any* of the five
...
def build_arg_parser():
    ...
    p.add_argument("--bag-relative", action="store_true",
                   help="Score a bag by how far its top slice stands out from its own "
                        ...
                        "comparable across exams (DOCUMENTATION.md 4.9).")
