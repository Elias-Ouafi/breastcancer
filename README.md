...
| Metadata catalogue | DuckDB + dbt, raw → staging → marts, 36 dbt tests, rebuilt in ~10 s |
| Quality gates | 222 tests (no GPU, no dataset), ruff, GitHub Actions |
| Stack | Python 3.12 · SQL · dbt · DuckDB · Parquet · pydicom · NumPy · pandas · PyTorch · Prefect · Flask · Docker |
...
| **Catalogue** | [catalog/](catalog/) | Tables, disk inventory, manifests and model scores joined in DuckDB. A dbt project builds the `stg` and `mart` layers, and 36 dbt tests fail the build on a label or key that disagrees with its source. The build is atomic. |
...
python -m catalog build     # ~10 s: 22,032 series, 5,060 patients, 36 dbt tests
python -m catalog query --file catalog/queries/02_annotated_patients_not_downloaded.sql
```

Every question about the data is a SQL query against tested tables. Python loads the
`raw` layer; a [dbt project](catalog/dbt/) builds the rest and tests it. Two examples
...
Schema, dbt tests, lineage graph and example queries: [docs/catalog.md](docs/catalog.md).
Why DuckDB and why dbt: [ADR 0009](docs/adr/0009-duckdb-metadata-catalogue.md),
[ADR 0010](docs/adr/0010-dbt-for-the-catalogue-transformations.md).
...
catalog/            DuckDB metadata catalogue: loader, dbt project, example queries
tests/              222 tests, synthetic fixtures, no GPU or dataset
...
| [docs/catalog.md](docs/catalog.md) | Metadata catalogue: build steps, dbt layers and tests, lineage, example queries |
...
pip install -e ".[dev]"
ruff check .
pytest              # 222 tests, no GPU or dataset needed
```
